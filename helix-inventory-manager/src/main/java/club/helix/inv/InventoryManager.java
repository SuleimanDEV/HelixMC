...
import club.helix.inv.opener.ChestInventoryOpener;
import club.helix.inv.opener.InventoryOpener;
import club.helix.inv.opener.SpecialInventoryOpener;
...
public class InventoryManager {
    ...
    private Map<UUID, HelixInventory> inventories;
    ...
    public List<Player> getOpenedPlayers(HelixInventory inv) {
    ...
    public Optional<HelixInventory> getInventory(Player p) {
        return Optional.ofNullable(this.inventories.get(p.getUniqueId()));
    }

    protected void setInventory(Player p, HelixInventory inv) {
        if (inv == null)
    ...
    public void handleInventoryOpenError(HelixInventory inventory, Player player, Exception exception) {
        inventory.close(player);

        Bukkit.getLogger().log(Level.SEVERE, "Error while opening SmartInventory:", exception);
    }

    public void handleInventoryUpdateError(HelixInventory inventory, Player player, Exception exception) {
        inventory.close(player);
    ...
    class InvListener implements Listener {
        ...
        public void onInventoryClick(InventoryClickEvent e) {
            ...
            if (clickedInventory == p.getOpenInventory().getTopInventory()) {
                ...
                HelixInventory inv = inventories.get(p.getUniqueId());
