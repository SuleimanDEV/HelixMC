...
import club.helix.inv.opener.InventoryOpener;
...
public class HelixInventory {
    ...
    private HelixInventory parent;

    private List<InventoryListener<? extends Event>> listeners;
    private InventoryManager manager;

    private HelixInventory(InventoryManager manager) {
        this.manager = manager;
    }

    public Inventory open(Player player) { return open(player, 0); }
    public Inventory open(Player player, int page) {
        Optional<HelixInventory> oldInv = this.manager.getInventory(player);
    ...
    public Optional<HelixInventory> getParent() { return Optional.ofNullable(parent); }
    ...
    public static final class Builder {
        ...
        public Builder parent(HelixInventory parent) {
            this.parent = parent;
            return this;
        }
        ...
        public HelixInventory build() {
            if(this.provider == null)
                throw new IllegalStateException("The provider of the HelixInventory.Builder must be set.");

            InventoryManager manager = this.manager != null ? this.manager : HelixInvsPlugin.manager();

            if(manager == null)
                throw new IllegalStateException("The manager of the HelixInventory.Builder must be set, "
                        + "or the SmartInvs should be loaded as a plugin.");

            HelixInventory inv = new HelixInventory(manager);
            inv.id = this.id;
